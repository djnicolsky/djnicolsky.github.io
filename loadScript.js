...
if(ModelScenario != 'none'){
    ...
    request.open("GET", "https://localhost/physics/scenarios/"+ModelScenario[0]+".model", false);
    request.send(null);
    ModelScript = request.responseText;

    if(ModelScenario[1] == 'y'){
      	request.open("GET", "https://localhost/physics/scenarios/"+ModelScenario[0]+".slider", false);
        request.overrideMimeType('text/plain; charset=x-user-defined');
   	    request.send(null);
    ...
    if(ModelScenario[2] == 'y'){
        request.open("GET", "https://localhost/physics/scenarios/"+ModelScenario[0]+".stream", false);
        request.overrideMimeType('text/plain; charset=x-user-defined');
        request.send(null);
