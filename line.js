...
Line.prototype.initialize = function(){
    ...
    for (var i=0; i<3; i++)
        this.traceLine.geometry.attributes.position.array[i]=AnimationScale*World.AxisScale[i]*this.traceXYZ[i];
    this.DisplayStep = 1;
...
Line.prototype.cleanup = function(){
	for (var i =  this.textLabels.length-1; i >= 0; i-- ){
		document.body.removeChild(this.textLabels[i].element);
	...
	this.traceGeo.removeAttribute('position');
...
Line.prototype.addLabel = function(){
	...
	//for (var ii =  0; ii < views.length; ii++ ) {
		...
		ptext.setParent(this.arrows[this.ti-1], camera, this.showlabel);
...
Line.prototype.draw = function(){
		...
		if(this.t<this.interval)
			for(var i=this.Ra.length-1; i>=0; i--)
				this.Ra.splice(i,1);
...
Line.prototype.restart = function(){
	...
	for (var ii=this.textLabels.length-1; ii>=0; ii-- ){
