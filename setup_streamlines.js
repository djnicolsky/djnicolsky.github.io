...
function restartLines(){
	var list = [];
	for(var i=listLines.length-1;i>=0;i--){
		var v = listLines[i].returnValues();
		list.push(v);
		listLines[i].cleanup();
		listLines.splice(i,1);
	}
	for(var i=list.length-1;i>=0;i--){
		var L = new Line(list[i]);
		L.initialize();
   		listLines.push(L);
   		list.splice(i,1);
	}
	...
	for(var i=0;i<listSurfaces.length;i++)
		listSurfaces[i].restart();
}
